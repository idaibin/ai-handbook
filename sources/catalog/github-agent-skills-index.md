...
- Raw GitHub search hits: `2229`
- Existing discovery-inbox candidates merged and re-verified: `4`
- Raw identities across all inputs: `2233`
- Unique repositories after case-insensitive `owner/repository` deduplication: `2107`
...
- Provisionally eligible for later deep analysis: `1693`
- Held as adjacent or unclear search hits: `414`

Composed machine-readable authority: [`github-agent-skills-index-latest.json`](github-agent-skills-index-latest.json).

The current manifest composes the previous verified `2087`-repository canonical manifest with page `20` of the `agent skills registry` discovery query.
...
- `"agent skills registry" in:name,description`, pages `1-20` processed at `20` results per page; page `21` was probed and returned `20` repositories, so the next merge boundary is page `21`.
...
Processed page `20` at `per_page=20`.

- Raw repository identities: `20`
- Internal batch duplicates: `0`
- Confirmed duplicates against the prior verified catalog in the directly inspected comparison sets: `0`
- Added as new repositories: `20`
- Previous verified catalog: `2087`
- Updated composed catalog: `2107`
- Added to provisional deep-analysis queue: `20`
- Added to held-for-review: `0`

All 20 page-20 identities were directly compared against persisted `agent-skills-registry` pages `12-19`, covering the active pagination window where earlier drift had produced repeated identities, and against the full persisted `skill-hub` pages `1-10` high-overlap artifact. No exact case-insensitive `owner/repository` match was found in those directly inspected sets. Earlier page batches remain chained into the canonical manifest, but this run does not claim a fresh direct re-read of every historical page. AI-handbook code-search emptiness was not used as uniqueness evidence because earlier runs demonstrated false negatives for known persisted identities.

All 20 page-20 results were provisionally classified from repository identity and GitHub search context as `skill_tooling`; all 20 are provisionally eligible for later deep analysis. Repository IDs, default branches, sizes, archived flags, result ordering, classifications, eligibility, and verification notes are stored in [`batches/agent-skills-registry-page-20.json`](batches/agent-skills-registry-page-20.json).
...
| `skill_tooling` | 777 | Identity indicates validation, linting, evaluation, testing, packaging, discovery, registry, marketplace, management, benchmark, SDK, template, fixture, or runtime tooling. |
...
- Previous canonical manifest commit: `1a22c94324999cccb15b60b740aa5ae74378683a` (`2087` repositories).
- `agent skills registry` page-20 batch commit: `29df5967c3d653e3a33244338f633e8c04bccb66`.
- Composed latest-manifest commit: `abda061e52035b603b3f97278772fc2269447aba`.
- `2087 + 20 = 2107` current unique repositories.
- `2233 - 126 = 2107` raw-to-unique reconciliation.
- Classification totals resolve to `2107`.
- `1693 + 414 = 2107`, matching the eligible and held partitions.
- Next index boundary: `"agent skills registry" in:name,description`, page `21`, `per_page=20`.
