...
- Raw GitHub search hits: `2329`
- Existing discovery-inbox candidates merged and re-verified: `4`
- Raw identities across all inputs: `2333`
- Unique repositories after case-insensitive `owner/repository` deduplication: `2201`
...
- Provisionally eligible for later deep analysis: `1787`
- Held as adjacent or unclear search hits: `414`

Composed machine-readable authority: [`github-agent-skills-index-latest.json`](github-agent-skills-index-latest.json).

The current manifest composes the previous verified `2181`-repository canonical manifest with page `25` of the `agent skills registry` discovery query.
...
- `"agent skills registry" in:name,description`, pages `1-25` processed at `20` results per page; page `26` was probed and returned `20` repositories, so the next merge boundary is page `26`.
...
Processed page `25` at `per_page=20`.
...
- Previous verified catalog: `2181`
- Updated composed catalog: `2201`
- Added to provisional deep-analysis queue: `20`
- Added to held-for-review: `0`

Page-25 identities were directly compared against persisted adjacent `agent-skills-registry` pages `19-24` and the full persisted `skill-hub` pages `1-10`, `agent-skills-hub` pages `1-2`, and `skill-registry` pages `1-10` high-overlap artifacts. No exact case-insensitive `owner/repository` duplicate was found in those directly inspected sets. Earlier catalog coverage is inherited through the verified `2181`-repository canonical manifest. AI-handbook code-search emptiness was not used as uniqueness evidence.

All 20 raw page-25 results were provisionally classified from repository identity and GitHub search context as `skill_tooling`; all 20 were added to the provisional deep-analysis queue. Repository IDs, default branches, sizes, archived flags, result ordering, classifications, eligibility, duplicate status, and verification notes are stored in [`batches/agent-skills-registry-page-25.json`](batches/agent-skills-registry-page-25.json).
...
| `skill_tooling` | 871 | Identity indicates validation, linting, evaluation, testing, packaging, discovery, registry, marketplace, management, benchmark, SDK, template, fixture, or runtime tooling. |
...
- Previous canonical manifest commit: `4dea6a663a83a596c51498fb15342a192d299fa0` (`2181` repositories).
- `agent skills registry` page-25 batch commit: `9e1fff9d1ca42ef34646164ae4fc6240e7725a28`.
- Composed latest-manifest commit: `6578101a2829ab312000d6dc610b44cc10ded360`.
- `2181 + 20 = 2201` current unique repositories.
- `2333 - 132 = 2201` raw-to-unique reconciliation.
- Classification totals resolve to `2201`.
- `1787 + 414 = 2201`, matching the eligible and held partitions.
- Next index boundary: `"agent skills registry" in:name,description`, page `26`, `per_page=20`.
