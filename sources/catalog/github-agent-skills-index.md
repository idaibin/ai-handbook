...
per_page=100
14 exact single-day shards with terminal pagination probes
```

The `2026-04-14` shard returned `23` repositories on page 1 and `0` on page 2. Recheck with `per_page=20` returned `20,3,0`. Case-insensitive `owner/repository` deduplication produced `23` unique identities and `0` internal duplicates.

April staging now contains `451` unique identities across `2026-04-01` through `2026-04-14`, with `0` cross-shard duplicates asserted.
...
| --- | ---: |
| `specification` | 0 |
| `skill_collection` | 246 |
| `single_skill_or_domain_package` | 130 |
| `awesome_index` | 1 |
| `skill_tooling` | 14 |
| `adjacent_search_hit` | 60 |
...
Current run report: [`runs/2026-08-08-agentskills-april-2026-04-14.md`](runs/2026-08-08-agentskills-april-2026-04-14.md).
...
Continue with the `2026-04-15` exact single-day shard and keep canonical totals frozen until reconciliation is complete.