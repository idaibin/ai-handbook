...
per_page=100
26 exact single-day shards with explicit terminal pagination probes
```

March staging through March 25 contained `655 / 655` identities. The `2026-03-26` shard contributed `16 / 16` additional case-insensitive unique identities with page counts `16, 0`. Exact `created:` day shards are disjoint by repository creation date, so cumulative March staging is now `671 / 671` identities with no intra-March duplicate asserted.

### March provisional classification through 2026-03-26

| Classification | Count |
| --- | ---: |
| `specification` | 0 |
| `skill_collection` | 532 |
| `single_skill_or_domain_package` | 50 |
| `awesome_index` | 3 |
| `skill_tooling` | 32 |
| `adjacent_search_hit` | 50 |
| `unclear_search_hit` | 4 |
| **Total** | **671** |

The March 26 contribution is `14 skill_collection`, `1 single_skill_or_domain_package`, and `1 adjacent_search_hit`. Classification is index-stage only and uses repository identity/name metadata; it is not a deep-content determination.
...
Current run report: [`runs/2026-08-07-agentskills-march-2026-03-26.md`](runs/2026-08-07-agentskills-march-2026-03-26.md).
...
Continue the deterministic March partition with the `2026-03-27` exact single-day shard. Keep canonical totals frozen until created-date partition unions are reconciled against unpartitioned staging and the complete historical canonical ledger.
