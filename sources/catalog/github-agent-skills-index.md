...
- Raw GitHub search hits: `2069`
- Existing discovery-inbox candidates merged and re-verified: `4`
- Raw identities across all inputs: `2073`
- Unique repositories after case-insensitive `owner/repository` deduplication: `1957`
...
- Provisionally eligible for later deep analysis: `1543`
- Held as adjacent or unclear search hits: `414`

Composed machine-readable authority: [`github-agent-skills-index-latest.json`](github-agent-skills-index-latest.json).

The current manifest composes the previous verified `1937`-repository canonical manifest with page `12` of the `agent skills registry` discovery query.
...
- `"agent skills registry" in:name,description`, pages `1-12` complete at `20` results per page; page `13` was probed and returned `20` repositories, so the next merge boundary is page `13`.
...
Processed page `12` at `per_page=20`.
...
- Previous verified catalog: `1937`
- Updated composed catalog: `1957`
- Added to provisional deep-analysis queue: `20`
- Added to held-for-review: `0`

All page-12 identities were directly compared against persisted `agent-skills-registry` pages `1-11`, plus directly inspected overlapping `skill-hub` pages `1-10`, `agent-skills-hub` pages `1-2`, and `skill-registry` pages `1-10` artifacts. No exact case-insensitive `owner/repository` identity was found in those inspected persisted sets.

Exact AI-handbook code search was also executed for all 20 page-12 identities and returned no positive prior identity match. Empty code-search results were not used as sole uniqueness proof because earlier runs demonstrated false negatives for known persisted identities.

The 20 new identities were provisionally classified only from repository identity and GitHub search context as `skill_tooling`. Repository IDs, default branches, sizes, archived flags, result ordering, classifications, eligibility, and verification notes are stored in [`batches/agent-skills-registry-page-12.json`](batches/agent-skills-registry-page-12.json).
...
| `skill_tooling` | 627 | Identity indicates validation, linting, evaluation, testing, packaging, discovery, registry, marketplace, management, benchmark, SDK, template, fixture, or runtime tooling. |
...
- Previous canonical manifest commit: `3f0d8c38c7831d0252d9904a8bbd214196725ef2` (`1937` repositories).
- `agent skills registry` page-12 batch commit: `e56ea1a59407482b2b62f8effdf6c4e6977ccfb5`.
- Composed latest-manifest commit: `7bb31f0016954ba596e8c43fbc0d2895c8ed7747`.
- `1937 + 20 = 1957` current unique repositories.
- `2073 - 116 = 1957` raw-to-unique reconciliation.
- Classification totals resolve to `1957`.
- `1543 + 414 = 1957`, matching the eligible and held partitions.
- Next index boundary: `"agent skills registry" in:name,description`, page `13`, `per_page=20`.
