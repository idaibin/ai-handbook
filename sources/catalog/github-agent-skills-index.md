...
- Raw GitHub search hits: `1046`
- Existing discovery-inbox candidates merged and re-verified: `4`
- Raw identities across all inputs: `1050`
- Unique repositories after case-insensitive `owner/repository` deduplication: `969`
- Exact duplicates removed across current inputs: `81`
- New unique repositories added in this run: `100`
- Provisionally eligible for later deep analysis: `655`
- Held as adjacent or unclear search hits: `314`
...
The focused `"openai skills" in:name,description` query collected pages `1-10`, with `10` repository identities per page.

- Raw repository identities: `100`
- Internal batch duplicates: `0`
- Already present in the current `869`-repository catalog: `0`
- Added as new repositories: `100`
- Updated composed catalog: `969`
...
| --- | ---: | ---: |
| `specification` | 0 | 0 |
| `skill_collection` | 80 | 80 |
| `single_skill_or_domain_package` | 11 | 11 |
| `awesome_index` | 0 | 0 |
| `skill_tooling` | 3 | 3 |
...
| `skill_collection` | 340 | Repository identity indicates a collection of Skills. |
| `single_skill_or_domain_package` | 75 | Repository identity indicates one Skill or a domain-focused package. |
| `awesome_index` | 36 | Repository identity indicates an index or curated collection. |
| `skill_tooling` | 203 | Repository identity indicates Skill discovery, validation, scanning, packaging, management, marketplace, catalog, registry, hub, directory, client, CLI, or runtime tooling. |
| `adjacent_search_hit` | 108 | Search hit is related to agents, OpenAI, MCP, Claude, Codex, or Skills but is not clearly a Skill repository from identity alone. |
| `unclear_search_hit` | 206 | Repository identity is insufficient for reliable classification. |
...
- OpenAI-skills batch commit: `3581e54c8622fdfc5d1fdc9b7334129fb7bfd3db`.
- Composed latest-manifest commit: `a1318f315e81a4d31ade7d291a2ca0ab4ef6ebd2`.
- The current batch contains `100` unique case-insensitive keys internally.
- Exact identity searches found `0` repositories in the current `869`-repository catalog.
- Prior `869` plus `100` new repositories resolves to `969`.
- Classification totals resolve to `969` repositories.
- `655 + 314 = 969`, matching the eligible and held partitions.
