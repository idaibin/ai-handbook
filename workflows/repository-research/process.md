...
## 5. Weekly usage quota gate

每次运行必须在创建 query、领取 candidate 或启动 runtime validation 之前读取平台提供的第一方周 usage 状态；长批次还要在索引 reconciliation 前、深度分析每组候选完成后复查。只接受可观测的精确数值和读取时间，不得从错误、延迟或体感推算额度。

统一字段：

- `remaining_percentage`：周剩余额度百分比；
- `observed_at` 与 `source`：读取时间与第一方状态来源。

唯一允许执行区间为：

```text
20 <= remaining_percentage < 99
```

任一条件成立即触发 `quota_stop`：

- `remaining_percentage < 20`：为当前周保留余量；
- `remaining_percentage >= 99`：视为周额度已重置，避免未知情况下消耗新周期额度；
- 精确周剩余额度不可观测：记录 `quota_not_observable`，因无法证明处于允许区间而按保守规则停止。

不得使用已用额度推导或替代上述判断，也不得伪造额度值、观测来源或阈值命中原因。
...
4. 如有变更，以原子 commit 推送到 `idaibin/ai-handbook:main`，并回读确认远端 commit、状态和指针一致；无变更时明确记录无需提交；
...
6. 回复停止类别、精确观测值（若可观测）、观测时间/来源、同步 commit 或无需提交、六条任务的暂停结果，以及 README 中的 Chat 恢复指令。
