...
- The cache reduces repeated reads, but it does not guarantee freshness; the caller must choose a TTL and an invalidation policy.
...
- 公开报告可以对外分享。
- 含有客户标识的附件不得对外分享；完成脱敏并获得审批后，该附件可以分享。
...
约束：仅允许读取候选 `SKILL.md`、当前任务必需的候选 references，以及 holdout 输入；禁止读取 criteria、基线 skill、工作区仓库和任何输出/结果。执行过程中未读取禁止项。
