...
- The cache does not guarantee freshness; the caller must choose a TTL and an invalidation policy.
...
约束：仅读取候选 skill 的 `SKILL.md`、当前任务所需 references，以及 holdout 输入；未读取 criteria、基线 skill、工作区仓库或任何输出/结果；仅写入本文件。
