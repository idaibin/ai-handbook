...
第二轮正式隔离生成。仍只允许读取 `<materialized>/hw-semantic-baseline/` 的 SKILL.md 及其对本任务明确要求的 references，以及 `<materialized>/hw-semantic-adversarial-inputs.md`；禁止读取候选 skill、工作区仓库、任何已有输出/结果/诊断。执行 T11–T16，把最终成品写入 `<run-output>/semantic-adversarial-baseline.md`，标题严格为 `## T11`…`## T16`，末尾记录完整任务约束和实际读取文件。使用 apply_patch；不要输出诊断。
