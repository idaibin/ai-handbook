...
- Source and CI design show strong production engineering signals, but nothing was executed in this review, so runtime correctness remains unvalidated.
- The framework exposes explicit approval waitpoints; prompt wording alone does not provide the same human gate.

## 约束与读取记录

- 约束：独立执行 T11–T16；除候选技能的 `SKILL.md`、当前任务明确要求的 references 和对抗输入外，不读取其他文件或任何结果；使用 `apply_patch` 写入指定文件。
- 实际读取：`<materialized>/hw-semantic-candidate/SKILL.md`、`<materialized>/hw-semantic-candidate/references/fact-integrity.md`、`<materialized>/hw-semantic-candidate/references/style-diagnostics.md`、`<materialized>/hw-semantic-candidate/references/quality-rubric.md`、`<materialized>/hw-semantic-adversarial-inputs.md`。
