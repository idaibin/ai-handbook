...
- 约束：仅读取指定 Skill、其为当前任务明确要求的 references，以及题目文件；未读取其他文件或任何结果；仅用 `apply_patch` 写入本文件。
- 实际读取文件：`<materialized>/hw-semantic-baseline/SKILL.md`、`<materialized>/hw-semantic-baseline/references/fact-integrity.md`、`<materialized>/hw-semantic-baseline/references/style-diagnostics.md`、`<materialized>/hw-semantic-baseline/references/quality-rubric.md`、`<materialized>/hw-semantic-adversarial-inputs.md`。
