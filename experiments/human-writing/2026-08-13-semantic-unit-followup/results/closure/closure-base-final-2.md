...
- 运行：正式 fixed-revision baseline closure 运行 2。
- 读取范围：只允许读取 `<materialized>/hw-semantic-baseline/` 的 `SKILL.md` 及其为当前任务明确要求的 references，以及 `<materialized>/hw-semantic-closure-inputs.md`。
- 禁止范围：禁止读取 criteria、候选、其他文件和任何结果。
- 执行范围：执行 R01–R06。
- 写入位置：`<run-output>/closure-base-final-2.md`。
- 格式：标题为 `## R01`…`## R06`，仅成品；末尾记录完整约束、baseline SHA 与实际读取文件；使用 `apply_patch`。
