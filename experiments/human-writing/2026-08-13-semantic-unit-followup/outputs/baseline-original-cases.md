...
- The repository shows strong production engineering signals in its source and CI design. Since this review executed nothing, runtime correctness and provider-service behavior remain below runtime_validated.
...
对于分号复合 locator，每个片段都必须匹配；Markdown `# heading` 或 `path.md#heading` 只会匹配 Markdown heading，其中 emoji 和空白可以归一；代码 symbol 按完整标识符匹配，自由文本则按明确的大小写/空白归一短语匹配。因此，`SECURITY.md#security` 不会仅因正文出现普通 security 一词而通过。
...
正式隔离生成任务。你只允许读取以下位置：`<materialized>/hw-semantic-baseline/` 中的 SKILL.md 及它明确要求为当前任务加载的 references，以及 `<materialized>/hw-semantic-inputs.md`。禁止读取工作区仓库、任何实验 cases/outputs/results/README、`<materialized>/hw-semantic-candidate` 或其他文件。先完整读取 SKILL.md，再按其规则选择引用；然后对输入文件 T01–T10 逐题执行。把每题最终成品写入 `<run-output>/semantic-baseline.md`，标题严格为 `## T01`…`## T10`。末尾加 `## Run metadata`，逐字记录本任务约束和实际读取文件。使用 apply_patch 创建文件；不要返回诊断或解释。
