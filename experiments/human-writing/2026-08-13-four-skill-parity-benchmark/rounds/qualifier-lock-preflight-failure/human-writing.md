...
- 对于同一个问题，优先保留一个主来源；其他来源用于考察不同实现、核对冲突或收集失败案例。
...
- Provider breadth and profile normalization can reduce adapter friction, but common interfaces do not establish equivalent behavior across every provider/native-tool combination.
- The repository's source and CI design show strong production engineering signals. Because this review executed nothing, runtime correctness and provider-service behavior remain below runtime_validated.

## T05

`--verify-remote` 通过已认证的 `gh api`，固定读取 commit、递归 tree 和 blob。它会校验 path/tree blob SHA，下载 blob 后重新计算 Git blob SHA，再检查内容中的 locator。

分号复合 locator 的每个片段都必须匹配。Markdown `# heading` 或 `path.md#heading` 只匹配 Markdown heading，其中 emoji 和空白可以归一；代码 symbol 按完整标识符匹配，自由文本按明确的大小写/空白归一短语匹配。因此，`SECURITY.md#security` 不会仅因正文出现普通 security 一词而通过。如果认证、网络或 GitHub 配额不可用，流程会以 `Not verified` 非零失败，不会静默跳过，也不会输出 token。
...
2. **Human-in-the-loop is often weaker than its label.** Activepieces and Trigger.dev provide explicit waitpoints. Several other candidates rely on prompt wording, LLM-interpreted free text, a debug pause or a user-operated pause button, so these mechanisms should not receive the same human-gate score.
...
Complete constraint: Use the fixed `human-writing` Skill at `/tmp/hw-round2b/human-writing` to complete T01–T10 in `/tmp/hw-round2b/generator-inputs.md`. Formal isolated generation: read only this Skill's SKILL.md and references it explicitly routes for these tasks, plus generator-inputs.md. Do not read any other `/tmp/hw-round2b` skill/output, workspace experiment, report, expected answer, prior result, or git history. Write finished artifacts to `/tmp/hw-round2b/outputs/human-writing.md` with headings exactly `## T01`…`## T10`, then `## Run metadata` containing this complete constraint and files actually read. Use apply_patch. Return artifact only; no analysis.

Files actually read:

- `/tmp/hw-round2b/human-writing/SKILL.md`
- `/tmp/hw-round2b/human-writing/references/fact-integrity.md`
- `/tmp/hw-round2b/human-writing/references/style-diagnostics.md`
- `/tmp/hw-round2b/human-writing/references/reasoning-and-explanation.md`
- `/tmp/hw-round2b/human-writing/references/quality-rubric.md`
