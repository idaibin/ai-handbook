...
- 进入课程前，需要先明确它将填补哪个 Output 缺口；完成后，需要更新来源卡、知识图谱以及实验/应用证据。
- 候选池可以很大，但新来源不再带来新模式或新证据时，就应停止扩张并转向实践。

## T03

- 该项目的恢复与停止设计主要针对实时语音会话中的瞬时失败、抢话以及媒体管线进行优化，不能被理解为面向任意长任务的可持久化 checkpoint。
- 房间权限不等同于 agent 工具授权；如果应用暴露高风险 function tools，还需要增加审批/策略层。
- 代码本体采用 Apache-2.0，但使用仓库所含或关联的 LiveKit 专有模型时，仍有必要单独审查 MODEL_LICENSE 中关于框架绑定与训练用途的限制。
...
`--verify-remote` 通过已经认证的 `gh api`，以固定方式读取 commit、递归 tree 与 blob。它会校验 path/tree blob SHA，下载 blob 后重新计算 Git blob SHA，并在内容中检查 locator。
...
Constraint: Use the fixed `human-writing` Skill at `/tmp/hw-round2c/human-writing` to complete T01–T10 in `/tmp/hw-round2c/generator-inputs.md`. Formal isolated generation: read only this Skill's SKILL.md and references it explicitly routes for these tasks, plus generator-inputs.md. Do not read any other `/tmp/hw-round2c` or `/tmp/hw-round2b` skill/output, workspace experiment, report, expected answer, prior result, or git history. Write finished artifacts to `/tmp/hw-round2c/outputs/human-writing.md` with headings exactly `## T01`…`## T10`, then `## Run metadata` containing this complete constraint and files actually read. Use apply_patch. Return artifact only; no analysis.

Files actually read:

- `/tmp/hw-round2c/human-writing/SKILL.md`
- `/tmp/hw-round2c/human-writing/references/fact-integrity.md`
- `/tmp/hw-round2c/human-writing/references/style-diagnostics.md`
- `/tmp/hw-round2c/human-writing/references/quality-rubric.md`
- `/tmp/hw-round2c/human-writing/references/content-modes.md`
- `/tmp/hw-round2c/generator-inputs.md`
