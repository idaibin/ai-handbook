...
- Complete constraint: Use the `humanizer` skill at `/tmp/hw-round2a/humanizer` to complete the writing tasks in `/tmp/hw-round2a/generator-inputs.md` as a formal isolated run. Read only that skill's `SKILL.md` and references it explicitly routes for these tasks, plus the generator input. Do not read any other `/tmp/hw-round2a` skill or output, workspace experiment, report, expected answer, or prior result. Write finished artifacts for T01 through T10 to `/tmp/hw-round2a/outputs/humanizer.md`, using headings exactly `## T01` through `## T10`, followed by `## Run metadata` containing the complete constraint and files actually read. Use `apply_patch`. Return artifacts only per task.
- Files actually read: `/tmp/hw-round2a/humanizer/SKILL.md`; `/tmp/hw-round2a/generator-inputs.md`.
- Additional references read: none.
