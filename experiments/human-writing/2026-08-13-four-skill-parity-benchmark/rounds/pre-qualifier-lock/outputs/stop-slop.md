...
Complete constraint: Use @stop-slop at `/tmp/hw-round2a/stop-slop` to complete the writing tasks in `/tmp/hw-round2a/generator-inputs.md`. This is a formal isolated run. Read only that Skill's SKILL.md and references it explicitly routes for these tasks, plus the generator input. Do not read any other `/tmp/hw-round2a` skill/output, workspace experiment, report, expected answer, or prior result. Write finished artifacts for T01–T10 to `/tmp/hw-round2a/outputs/stop-slop.md`, headings exactly `## T01`…`## T10`, then `## Run metadata` containing this complete constraint and files actually read. Use apply_patch. Return artifacts only per task.

Files actually read:

- `/tmp/hw-round2a/stop-slop/SKILL.md`
- `/tmp/hw-round2a/stop-slop/references/phrases.md`
- `/tmp/hw-round2a/stop-slop/references/structures.md`
- `/tmp/hw-round2a/stop-slop/references/examples.md`
