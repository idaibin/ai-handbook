...
An earlier same-model generation used fresh isolated agents for all four skills. Its three comparator outputs were then held fixed across both the preceding `human-writing` revision comparison and the final release comparison; each changed `human-writing` revision received its own fresh isolated generation agent. This avoids adding comparator sampling noise while ensuring each changed Skill is newly executed. Every generation used the same task:
...
The assigned skill trees and commits are in `manifest.yaml`. `generator-inputs.md` contains neutral IDs and omits source paths, references, degradation/control labels, and expected decisions. All three final judge packets and judge sheets were then regenerated from the fixed four-output set.
