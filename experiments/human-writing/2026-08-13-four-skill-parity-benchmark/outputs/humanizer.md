...
- Constraint: Use the fixed `humanizer` Skill at `/tmp/hw-round2b/humanizer` to complete T01–T10 in `/tmp/hw-round2b/generator-inputs.md`. Formal isolated generation: read only that Skill's SKILL.md and references it explicitly routes for these tasks, plus generator-inputs.md. Do not read any other `/tmp/hw-round2b` skill/output, workspace experiment, report, expected answer, prior result, or git history. Write finished artifacts to `/tmp/hw-round2b/outputs/humanizer.md` with headings exactly `## T01`…`## T10`, then `## Run metadata` containing this complete constraint and files actually read. Use apply_patch. Return artifact only; no analysis.
- Files actually read: `/tmp/hw-round2b/humanizer/SKILL.md`; `/tmp/hw-round2b/generator-inputs.md`.
