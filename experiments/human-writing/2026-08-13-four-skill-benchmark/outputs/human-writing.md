...
任务约束：正式独立生成。只允许读取两个位置：materialized skill human-writing/（先完整读 SKILL.md，并按其规则读所需 references）和 generator-inputs.md。禁止读取 ai-handbook 该实验的 cases、outputs、results、README 或其他目录。按 T01–T10 各自 Instruction/Input 生成。用 apply_patch 覆盖 outputs/human-writing.md，标题 `## T01`…`## T10`，末尾 Run metadata 写本条完整任务约束和实际读取文件。

实际读取文件：

- materialized skill human-writing/SKILL.md
- materialized skill human-writing/references/fact-integrity.md
- materialized skill human-writing/references/style-diagnostics.md
- materialized skill human-writing/references/quality-rubric.md
- generator-inputs.md
