...
- 完整任务约束：正式独立生成。只允许读取两个位置：materialized skill stop-slop/（先完整读 SKILL.md 和明确引用）和 generator-inputs.md。禁止读取 ai-handbook 该实验的 cases、outputs、results、README 或其他目录。按 T01–T10 各自 Instruction/Input 生成。用 apply_patch 覆盖 outputs/stop-slop.md，标题 `## T01`…`## T10`，末尾 Run metadata 写本条完整任务约束和实际读取文件。
- 实际读取文件：materialized skill stop-slop/SKILL.md；materialized skill stop-slop/references/phrases.md；materialized skill stop-slop/references/structures.md；materialized skill stop-slop/references/examples.md；generator-inputs.md。
